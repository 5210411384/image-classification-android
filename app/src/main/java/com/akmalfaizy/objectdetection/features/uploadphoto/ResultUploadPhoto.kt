...
import com.akmalfaizy.objectdetection.features.takephoto.DataClassResult
...
class ResultUploadPhoto : AppCompatActivity() {

    private lateinit var recyclerView : RecyclerView
    private lateinit var listDataUploadPhoto :ArrayList<DataClassResult>
    private lateinit var progressBar : ProgressBar
    private lateinit var textNotFound : TextView

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_result_upload_photo)

        listDataUploadPhoto = ArrayList()
    ...
    private fun searchData(searchQuery: String) {
        val adapter = AdapterUpload(this, listDataUploadPhoto)
        ...
        AndroidNetworking.get(url)
            ...
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject) {
                    try {
                        ...
                        for (i in 0 until organicResultsArray.length()) {
                            ...
                            listDataUploadPhoto.add(DataClassResult(title, displayed_link, snippet))
                            adapter.notifyDataSetChanged()
                            progressBar.visibility = View.GONE

                            if (listDataUploadPhoto.isEmpty()) {
                ...
                override fun onError(anError: ANError?) {
                    Toast.makeText(this@ResultUploadPhoto, "No Result found for the search query..", Toast.LENGTH_SHORT).show()
                    progressBar.visibility = View.GONE
                }
            })
    }
