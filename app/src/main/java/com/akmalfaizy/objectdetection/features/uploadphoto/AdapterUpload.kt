...
class AdapterUpload(private val context: ResultUploadPhoto, private val listDataUploadPhoto: ArrayList<DataClassResult>) : RecyclerView.Adapter<AdapterUpload.MyViewHolder>(){
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): AdapterUpload.MyViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.search_result_rv, parent, false)
        return MyViewHolder(itemView)
    }
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        val currentItem = listDataUploadPhoto[position]
    ...
    override fun getItemCount(): Int {
        return listDataUploadPhoto.size
    }
